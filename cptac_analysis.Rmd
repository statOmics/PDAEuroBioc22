...
pe_all$condition <- pe_all[["peptideRaw"]] %>% colnames %>% substr(12, 12)
pe_all$spikeConcentration <- rep(c(A = 0.25, B = 0.74, C = 2.22, D = 6.67, E = 20),
                                 each = 9)
```
...
```{r}
pe <- filterFeatures(pe, ~ pe_na$nNArows$nNA <= 4)
nrow(pe[["peptideLog"]])
```
